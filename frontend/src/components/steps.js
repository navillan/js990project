...
export default function StepSelector() {
  ...
  const phonePattern = /^(\+90|0)?5[0345][0-9]{8}$/g; // +905xxx, 05xxx, 5xxx
...
const handlePhoneChange = (e) => {
  ...
  const sendRequest = async () => {
    const data = { beklemeListesi, selectedDate, selectedTime, selectedGuest, selectedName, selectedEmail, selectedPhone };
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Rezervasyon başarılı bir şekilde alındı.');
      }
    } catch (error) {
      alert('Network error');
    }
